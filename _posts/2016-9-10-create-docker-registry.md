...
使用 `docker registry/v2` 搭建私有镜像仓库(不加TSL认证的)。
...
并以
`-v ~/certs:/certs -e REGISTRY_HTTP_TLS_CERTIFICATE=/certs/domain.crt -e REGISTRY_HTTP_TLS_KEY=/certs/domain.key` 参数启动registry container, 即为`Secure Registry`。
